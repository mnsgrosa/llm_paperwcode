...
class Researcher:
    ...
    async def get_context(self, query: str):
        try:
            ...
            if topic in self.context:
                ...
                for structured_paper in structured_papers:
                    self.context[topic].append(structured_paper)
                return self.context[topic]
    ...
    async def run(self, user_input: str):
        context_query = await self.get_context(user_input)
        
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are a helpful AI research assistant."),
            ("system", f"Context from papers: {context_query}"),
            ("system", f"here is the last 5 pair of user prompt and assistant response {self.history[-5:]}")
            ("human", "{input}")
        ])

        response = await self.llm.ainvoke(prompt.invoke({"input": user_input}))
        self.history.append((user_input, response.content))
